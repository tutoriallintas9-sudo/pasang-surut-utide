...
if uploaded_file is not None and run_analysis:
    ...
    decompose_utide = solve(tanggal, sensor, lat=latitude,
                            constit=constituents, nodal=True,
                            trend=True, method='ols', conf_int='linear')
    ...
    **Kategori Formzahl (F):**
    ...
    MSL_rec = np.mean(h_out_predutide)
    HWS_rec = np.max(h_out_predutide)
    LWS_rec = np.min(h_out_predutide)
    ...
    # =========================================================
    # TABEL ELEVASI PENTING
    # =========================================================

    df_elevasi_rekonstruksi = pd.DataFrame({
        'Parameter': [
            'MSL',
            'HWS',
            'LWS',
        ...
        'Keterangan': [
            'Rata-rata muka air hasil rekonstruksi harmonik',
            f'Muka air tertinggi; terjadi pada {waktu_HWS}',
            f'Muka air terendah; terjadi pada {waktu_LWS}',
            'Selisih antara HWS dan LWS'
        ]
    })

    st.subheader(
        "Tabel Elevasi Penting Berdasarkan Rekonstruksi Harmonik"
    )

    st.dataframe(
        df_elevasi_rekonstruksi.style.format({
            'Elevasi (m)': '{:.3f}'
        }),
